...
const AgentWorkspace:React.FC=()=>{
  ...
  const send=async()=>{
    ...
    try{
      ...
      const result=await r.json().catch(()=>({})); if(!r.ok)throw new Error(result.error||`Agent failed (${r.status})`); if(!result.resumeData)throw new Error('Agent returned no resume.');
  ...
  return <div className="h-screen w-full bg-background flex overflow-hidden">
    ...
    <main className="flex-1 min-w-0 flex flex-col relative">
      ...
      <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-background via-background to-transparent pt-8 pb-4 px-3">
        <div className="max-w-3xl mx-auto border rounded-3xl bg-card shadow-lg p-2">
          ...
          <Textarea value={prompt} onChange={e=>setPrompt(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send();}}} placeholder="Message Resume Agent… paste a JD or ask for any resume change" className="border-0 shadow-none focus-visible:ring-0 min-h-[72px] max-h-48 resize-none"/>
          <div className="flex justify-between items-center px-1 pb-1"><div className="flex gap-1"><Button size="icon" variant="ghost" onClick={()=>fileRef.current?.click()} title="Attach reference"><Paperclip className="w-4 h-4"/></Button><Button size="sm" variant="ghost" onClick={()=>importRef.current?.click()}>Upload resume</Button></div><Button size="icon" className="rounded-full" disabled={!prompt.trim()||loading} onClick={send}>{loading?<Loader2 className="w-4 h-4 animate-spin"/>:<Send className="w-4 h-4"/>}</Button></div>
        </div>
      </div>
      {resumeOpen&&<aside className="resume-artifact-sidebar w-[min(560px,42vw)] min-w-[380px] shrink-0 border-l bg-card flex flex-col shadow-xl z-20">
        ...
        <div className="flex-1 min-h-0 overflow-auto">
          {resumeEditMode ? <div className="p-3"><ResumeForm activePanel="form"/></div> : <div className="p-4"><div className="resume-artifact-paper"><ResumePreview/></div></div>}
        </div>
      </aside>}
